...
#list = [[0,2,2,2], [2,0,2,0], [0,8,0,0], [2,16,0,0]] # список для проверки работы функций 
...
def lost(list):
    '''
    Функция определяет проиграл пользователь или нет
    В случае проигрыша возвращает 0
    Во всех остальных случаях положительное число.
    list - список со значениями плиток
    '''
...
def win(list):
    '''
    Функция определяет выиграл пользователь или нет.
    Ели да - возращает True, нет - False
    list - список со значениями плиток
    '''
    victory = False
    for i in range(4):
